...
echo "Author: Ulrik Vinther-Jensen"
echo "----------------------------------------------------------"
echo
...
perform_package_option "2.2.XXX Ensure HTTP PROXY is not installed (Automated)" "purge" "squid"
perform_package_option "2.2.14 Ensure SNMP Server is not enabled (Automated)" "disable" "snmpd snmp"
perform_package_option "2.2.14 Ensure Samba Server is not enabled (Automated)" "purge" "samba"
...
perform_command_option "4.2.1.5 Ensure journald is not configured to send logs to rsyslog" "sudo sed -i '/ForwardToSyslog=yes/d' /etc/systemd/journald.conf"
perform_command_option "4.2.1.6 Ensure journald log rotation is configured per site policy" "sudo sed -Ei 's/^#?(\s*SystemMaxUse\s*=).*/\1=500M/; s/^#?(\s*SystemKeepFree\s*=).*/\1=100M/; s/^#?(\s*RuntimeMaxUse\s*=).*/\1=50M/; s/^#?(\s*RuntimeKeepFree\s*=).*/\1=20M/; s/^#?(\s*MaxFileSec\s*=).*/\1=1week/' /etc/systemd/journald.conf"
...
perform_command_option "6.2.9 Ensure root PATH Integrity (Automated)" "RPCV=\"\$(sudo -Hiu root env | grep '^PATH' | cut -d= -f2)\"; echo \"\$RPCV\" | grep -q \"::\" && echo \"root's path contains an empty directory (::)\"; echo \"\$RPCV\" | grep -q \":\$\" && echo \"root's path contains a trailing (:)\" ; for x in \$(echo \"\$RPCV\" | tr \":\" \" \"); do if [ -d \"\$x\" ]; then ls -ldH \"\$x\" | awk '\$9 == \".\" {print \"PATH contains current working directory (.)\"} \$3 != \"root\" {print \$9, \"is not owned by root\"} substr(\$1,6,1) != \"-\" {print \$9, \"is group writable\"} substr(\$1,9,1) != \"-\" {print \$9, \"is world writable\"}'; else echo \"\$x is not a directory\"; fi done ; echo \"Please Correct or justify any items discovered. If nothing is shown\""
...
perform_command_option "6.2.11" "valid_shells=\"^(\$(sed -rn '/^\\//{s,/,\\\\\\\\/,g;p}' /etc/shells | paste -s -d '|' -))\$\"; awk -v pat=\"\$valid_shells\" -F: '\$(NF) ~ pat { print \$1 \" \" \$(NF-1) }' /etc/passwd | while read -r user home; do if [ ! -d \"\$home\" ]; then echo -e \"\n- User \"\$user\" home directory \"\$home\" doesn't exist\n- creating home directory \"\$home\"\n\"; mkdir \"\$home\"; chmod g-w,o-wrx \"\$home\"; chown \"\$user\" \"\$home\"; fi; done"
perform_command_option "6.2.12" "output=\"\"; valid_shells=\"^(\$(sed -rn '/^\\//{s,/,\\\\\\\\/,g;p}' /etc/shells | paste -s -d '|' -))\$\"; awk -v pat=\"\$valid_shells\" -F: '\$(NF) ~ pat { print \$1 \" \" \$(NF-1) }' /etc/passwd | while read -r user home; do owner=\"\$(stat -L -c \"%U\" \"\$home\")\"; if [ \"\$owner\" != \"\$user\" ]; then echo -e \"\n- User \"\$user\" home directory \"\$home\" is owned by user \"\$owner\"\n - changing ownership to \"\$user\"\n\"; chown \"\$user\" \"\$home\"; fi; done"
